...
if __name__ == '__main__':
    ...
    APPID = 'LYH.DrawEnergyDiagram.3.0'
...
class myWidget(Ui_Draw_Energy_Diagram_Form, Qt.QWidget, Qt_Widget_Common_Functions):
    ...
    def xlsx_dropped(self, xlsx_filename, called_from_update=False):
        ...
        self.y_s = [state[1] for route in self.routes if 'TAG' not in route[0][1] for state in route if isinstance(state[1], float) or isinstance(state[1], int)]
        min_energy = min([energy for energy in self.y_s])
        max_energy = max([energy for energy in self.y_s])
    ...
    def load_xlsx(self, filename):
        ...
        # supplement data and tag according to dimension of date
